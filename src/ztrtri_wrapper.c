                    ...
                    int *INFO )
{
    ...
    if ( uplo == dplasmaLower ) {
        ...
        dplasma_add2arena_lower( &((parsec_ztrtri_L_taskpool_t*)parsec_trtri)->arenas_datatypes[PARSEC_ztrtri_L_LOWER_TILE_ADT_IDX],
        ...
        /* Lower part of A with diagonal part */
        dplasma_add2arena_upper( &((parsec_ztrtri_U_taskpool_t*)parsec_trtri)->arenas_datatypes[PARSEC_ztrtri_U_UPPER_TILE_ADT_IDX],
                                 A->mb*A->nb*sizeof(dplasma_complex64_t),
                                 PARSEC_ARENA_ALIGNMENT_SSE,
                                 parsec_datatype_double_complex_t, A->mb, 1 );
    }

    dplasma_add2arena_tile( &((parsec_ztrtri_L_taskpool_t*)parsec_trtri)->arenas_datatypes[PARSEC_ztrtri_L_DEFAULT_ADT_IDX],
                            A->mb*A->nb*sizeof(dplasma_complex64_t),
                            PARSEC_ARENA_ALIGNMENT_SSE,
...
dplasma_ztrtri_Destruct( parsec_taskpool_t *tp )
{
    ...
    dplasma_matrix_del2arena( &otrtri->arenas_datatypes[PARSEC_ztrtri_L_DEFAULT_ADT_IDX   ] );
    dplasma_matrix_del2arena( &otrtri->arenas_datatypes[PARSEC_ztrtri_L_LOWER_TILE_ADT_IDX] ); // lower and upper have the same index
