...
int
parsec_core_zgemm_hip(parsec_device_gpu_module_t* gpu_device,
                      parsec_gpu_task_t*          gpu_task,
                      parsec_gpu_exec_stream_t*   gpu_stream)
{
    int transA, transB;
...
#if defined(PRECISION_z) || defined(PRECISION_c)
    hipblasDoubleComplex alphag, betag;
    alphag.x = creal(alpha); alphag.y = cimag(alpha);
    betag.x  = creal(beta);  betag.y  = cimag(beta);
#else
    double alphag = alpha;
    double betag  = beta;
#endif
...
parsec_dtd_create_zgemm_task_class(parsec_taskpool_t* dtd_tp, int tile_full, int devices)
{
    ...
    if( devices & PARSEC_DEV_HIP )
        parsec_dtd_task_class_add_chore(dtd_tp, zgemm_tc, PARSEC_DEV_HIP, parsec_core_zgemm_hip);
#endif
