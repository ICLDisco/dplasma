    ...
    for( k = *iteration; k < total; k++, *iteration += 1, count++ ) {
        ...
        for( n = k+1; n < dcA->super.nt; n++, count++ ) {
            ...
            parsec_dtd_taskpool_insert_task( dtd_tp,      parsec_core_unmqr,          0,    "unmqr",
                               ...
                               sizeof(int),           &tempkm,                            PARSEC_VALUE,
                               sizeof(int),           &ib,                                PARSEC_VALUE,
                               PASSED_BY_REF,         PARSEC_DTD_TILE_OF(A, k, k),      PARSEC_INPUT | TILE_LOWER,
                               sizeof(int),           &ldak,                              PARSEC_VALUE,
        ...
        for( m = k+1; m < dcA->super.mt; m++, count++ ) {
            ...
            parsec_dtd_taskpool_insert_task( dtd_tp,      parsec_core_tsqrt,
                              (dcA->super.mt-k)*(dcA->super.mt-k)*(dcA->super.mt-k),  "tsqrt",
                               ...
                               sizeof(int),           &tempkn,                            PARSEC_VALUE,
                               sizeof(int),           &ib,                                PARSEC_VALUE,
                               PASSED_BY_REF,         PARSEC_DTD_TILE_OF(A, k, k),     PARSEC_INOUT | TILE_UPPER,
                               sizeof(int),           &ldak,                              PARSEC_VALUE,
...
int main(int argc, char ** argv)
{
    ...
    dplasma_add2arena_lower( &parsec_dtd_arenas_datatypes[TILE_LOWER],
                             dcA.super.mb*dcA.super.nb*sizeof(dplasma_complex64_t),
                             PARSEC_ARENA_ALIGNMENT_SSE,
                             parsec_datatype_double_complex_t, dcA.super.mb, 0 );

    /* Upper triangular part of tile with diagonal */
    dplasma_add2arena_upper( &parsec_dtd_arenas_datatypes[TILE_UPPER],
                             dcA.super.mb*dcA.super.nb*sizeof(dplasma_complex64_t),
                             PARSEC_ARENA_ALIGNMENT_SSE,
    ...
    *iteration = 0;
    ...
    parsec_dtd_taskpool_insert_task( dtd_tp,       insert_task_geqrf, 0, "insert_task_geqrf",
                       ...
                       sizeof(two_dim_block_cyclic_t *), &dcA,      PARSEC_REF,
                       sizeof(two_dim_block_cyclic_t *), &dcT,      PARSEC_REF,
                       PARSEC_DTD_ARG_END );
    ...
    dplasma_matrix_del2arena( &parsec_dtd_arenas_datatypes[TILE_LOWER] );
    dplasma_matrix_del2arena( &parsec_dtd_arenas_datatypes[TILE_UPPER] );
