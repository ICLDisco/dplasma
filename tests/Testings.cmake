...
foreach(prec ${DPLASMA_PRECISIONS} )
  ...
  if (DPLASMA_HAVE_CUDA)
    ...
    # The hostsolve twin keeps the triangular solve on the CPU and the trailing
    # updates on the device, so a panel crosses the bus twice per step. That is
    # what TRSM did before it had a GPU solve, and it is the only thing here
    # that exercises those transfers, so it is worth keeping on purpose.
    dplasma_add_test(trsm               trmm    1gpu_cuda_shm -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -- --mca device_cuda_memory_number_of_blocks 4096)
    dplasma_add_test(trsm               trmm    1gpu_cuda_hostsolve_shm -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 --trsm_gpu_solve 0 -- --mca device_cuda_memory_number_of_blocks 4096)
  ...
  if (DPLASMA_HAVE_HIP)
    ...
    dplasma_add_test(trsm               trmm    1gpu_hip_shm -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -- --mca device_hip_memory_number_of_blocks 4096)
    dplasma_add_test(trsm               trmm    1gpu_hip_hostsolve_shm -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 --trsm_gpu_solve 0 -- --mca device_hip_memory_number_of_blocks 4096)
...
if( MPI_C_FOUND )
  ...
  foreach(prec ${DPLASMA_PRECISIONS})
    ...
    if (DPLASMA_HAVE_CUDA AND MPI_C_FOUND)
        ...
        dplasma_add_test(trsm  trmm       1gpu_cuda_mpi:${PROCS} -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -P 2 -- --mca device_cuda_memory_number_of_blocks 4096)
        dplasma_add_test(trsm  trmm       1gpu_cuda_hostsolve_mpi:${PROCS} -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -P 2 --trsm_gpu_solve 0 -- --mca device_cuda_memory_number_of_blocks 4096)
    ...
    if (DPLASMA_HAVE_HIP AND MPI_C_FOUND)
        ...
        dplasma_add_test(trsm  trmm       1gpu_hip_mpi:${PROCS} -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -P 2 -- --mca device_hip_memory_number_of_blocks 4096)
        dplasma_add_test(trsm  trmm       1gpu_hip_hostsolve_mpi:${PROCS} -M 1280 -N 1280 -t 320 ${OPTIONS} -g 1 -P 2 --trsm_gpu_solve 0 -- --mca device_hip_memory_number_of_blocks 4096)
