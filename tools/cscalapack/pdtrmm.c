...
int main( int argc, char **argv ) {
    ...
    // dplasma_dplrnt( parsec, 0, (parsec_tiled_matrix_dc_t *)&dcC, Cseed );
...
#ifdef VERBOSE
      {
        ...
        int nout=6;
        pdlaprnt_(&Am, &An, A, &i1, &i1, descA, &i0, &i0, nameA, &nout, pwork);
        pdlaprnt_(&Bm, &Bn, B, &i1, &i1, descB, &i0, &i0, nameB, &nout, pwork);
        free(pwork);
      }
#endif
  ...
  #else
          if( 0 == iam ) {
              printf("[****] TIME(s) %12.5f : dtrmm \tPxQ= %3d %-3d NB= %4d N= %7d : %14f gflops"
                    " - ENQ&PROG&DEST %12.5f : %14f gflops"
                    " - ENQ %12.5f - DEST %12.5f\n",
                            telapsed, nprow, npcol, nb, n,
                            gflops,
